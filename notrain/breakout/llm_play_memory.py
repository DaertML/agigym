...
# --- History Management ---
HISTORY_LENGTH = 3  # Number of past steps to remember for Breakout
history_buffer = deque(maxlen=HISTORY_LENGTH)
# --- End History Management ---

# Initialize the plot outside the loop
fig, ax = plt.subplots(figsize=(10, 8)) # Increased size for Breakout
img_plot = ax.imshow(env.render())
ax.set_title(f"ALE/Breakout-v5 | Step: {step} | Reward: {total_reward}")
...
while not terminated and not truncated and step < 500: # Increased steps for Breakout
    step += 1
    print(f"\n--- Step {step} ---")
    ...
    ax.set_title(f"ALE/Breakout-v5 | Step: {step} | Current Reward: {reward if 'reward' in locals() else 0} | Total Reward: {total_reward}")
    plt.draw()
    plt.pause(0.001)

    filename = f'breakout_step_{step}.png'
    ...
    prompt_content = f"""
Current Breakout Environment State:
  Observation (partial): {np.round(observation.flatten()[:20], 2)}... (shape: {observation.shape})

{history_str}

Based on the current game state and past history, what action should the player take to break the bricks and keep the ball in play?
The possible actions are:
2: move right
3: move left

Try to reason about the best action to take to maximize your score and prevent the ball from going out.
Write the chosen action in boxed LaTeX format, for example: $\boxed{2}$
"""
    ...
    try:
        ...
        if extracted_action is not None and 0 <= extracted_action <= 3:
        ...
        time.sleep(0.05)  # Adjust delay as needed for Breakout
